...
{content}"""
            ...
            result = await self._run_gbrain_command(
                ["put", file_path.stem, str(temp_file), "--source", self.source]
            )
    ...
    async def get_idea_graph(self, slug: str, depth: int = 2) -> Dict:
        ...
        try:
            result = await self._run_gbrain_command(
                ["graph", slug, "--depth", str(depth), "--source", self.source]
            )
    ...
    async def create_link(
        self, from_slug: str, to_slug: str, link_type: str = "evolves_into"
    ) -> Dict:
        """
        ...
        try:
            result = await self._run_gbrain_command(
                ["link", from_slug, to_slug, "--type", link_type, "--source", self.source]
            )
    ...
    async def get_idea_details(self, slug: str) -> Dict:
        ...
        """
        try:
            result = await self._run_gbrain_command(["get", slug, "--source", self.source])
            ...
            if line.strip() and not line.startswith("["):
                ...
                if "] " in line and "--" in line:
                    try:
                        ...
                        ideas.append({"slug": slug, "score": score, "snippet": snippet})
    ...
    def _parse_gbrain_graph_result(self, result: str) -> Dict:
        ...
        return {"status": "success", "raw_output": result, "timestamp": datetime.now().isoformat()}
