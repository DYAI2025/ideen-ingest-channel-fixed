...
async def search_ideas(
    ...
    phase: Optional[str] = Query(
        None, description="Filter by phase (seed, sprout, growth, flower, harvest)"
    ),
):
    """
    ...
    try:
        ...
        return JSONResponse(
            content={"query": query, "phase": phase, "results": ideas, "count": len(ideas)}
        )
...
@router.get("/list")
async def list_ideas(phase: Optional[str] = Query(None, description="Filter by phase")):
    """
    ...
    try:
        ...
        return JSONResponse(content={"phase": phase, "results": ideas, "count": len(ideas)})
...
async def get_idea_graph(slug: str, depth: int = Query(2, description="Graph traversal depth")):
    """
